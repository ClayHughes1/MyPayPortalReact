...
export default function usePaymentSources() {
     ...
     * ---------------------------------------------------------
     *
     * The authenticated API determines the CustomerId from
     * the authenticated user's NameIdentifier claim.
     *
     * We only use the local user here for validation/logging.
     */
        ...
        if (!customerId) {
            ...
            logService.logWarning(
                "Payment source list could not be loaded because no valid customer ID was available.",
                {
                    ...
                    action:
                        "PAYMENT_SOURCE_LIST_LOAD_VALIDATION_FAILED"
                }
            );
...
console.log('loading payment sources.  \n');
        try {
            ...
            await logService.info(
                `Customer ${customerId} requested payment sources.`,
                {
                    ...
                    action:
                        "PAYMENT_SOURCE_LIST_LOAD_STARTED"
                }
            );
             ...
             *
             * The API determines the customer from the
             * authenticated user's NameIdentifier claim.
             */

            // const response =
            //     await paymentSourceService.getAll(customerId);

            const response =
                await paymentSourceService.getAll();

            // console.log("Payment Source response ..  \n",response);
    ...
    const createPaymentSource = async (
        ...
        try {
            ...
            /*
             * Do NOT add customerId here.
             *
             * The API gets it from the authenticated user.
             */
